...
(mockados) podem ficar ✅ enquanto a validação manual no tenant permanece ⬜ e rastreada.
...
3. **Filtro por extended property (US-1.10).** Validar a query `$filter`/`$expand` sobre
   `"SystemTime 0x3FEF"` na pasta drafts no tenant real; se o `$filter` por valor não for
   fiável, filtrar por **presença** + "futuro" client-side.
...
## Onde estarão os testes (proposta)

- Unit: estender `tests/unit/test_graph_email_client.py` (montagem do `singleValueExtendedProperties`
  no draft; query da listagem) e `test_audit.py` (`email.schedule`/`email.schedule_cancel`).
- Integração (tools ponta-a-ponta): estender `tests/integration/test_email_write_e2e.py`
  (agendar/cancelar + transversais) e `test_email_read_e2e.py` (listar), com o
