...
| **US-3.6** | Envio a contacto explícito é estritamente 1:1 (barreira server-side) | **Medida de segurança crítica.** Quando o utilizador pede explicitamente para enviar a UMA PESSOA (por nome/email), a mensagem NUNCA pode acabar num chat de **grupo** onde essa pessoa coexista com terceiros — só na conversa **1:1** com essa pessoa. `teams_send_message_prepare` aceita um parâmetro **opcional** `intended_recipient` (email do contacto explicitamente pedido). Se presente, o servidor **exige** que o `chat_id` alvo seja um `oneOnOne` cujo ÚNICO outro membro (excluído o próprio) == `intended_recipient` (case-insensitive); caso contrário (grupo, 1:1 com outra pessoa, ou membro alvo sem email a impedir a verificação) **recusa com `error` e NÃO emite `confirmation_token`** (sem token = sem envio possível). Reforça-se o **caminho fácil seguro** (US-3.4 já resolve sempre o 1:1 via get-or-create) e as descrições das tools instruem o LLM a passar sempre `intended_recipient` quando o pedido nomeia uma pessoa. Ver **D12**. |
...
- **D12 — Barreira anti-fuga no envio a contacto explícito (US-3.6).** Quando o pedido nomeia uma pessoa, como garantir que a mensagem nunca vai para um grupo onde essa pessoa esteja com terceiros? Opções: (A) só endurecer descrições das tools — defesa prompt-level, **não garante** (o LLM continua a poder escolher um `chat_id` de grupo); (B) **guarda server-side aplicável** — `teams_send_message_prepare` aceita `intended_recipient` opcional e, se presente, recusa qualquer `chat_id` que não seja um `oneOnOne` exatamente com essa pessoa, sem emitir token; (C) tornar o caminho seguro o caminho fácil — US-3.4 (`get_or_create_one_on_one_chat`) já resolve sempre o 1:1 e nunca devolve um `chat_id` de grupo. *Recomendação: **B + C + descrições (A)**. (C) já existe e cobre o caso feliz; (B) é a única garantia **aplicável pelo servidor** que fecha o caminho `teams_send_message` com `chat_id` arbitrário — não depende da obediência do modelo; (A) reduz a probabilidade de o LLM sequer tentar. `intended_recipient` é **opcional** por retrocompatibilidade e para não bloquear o envio legítimo a grupos (quando ausente, o comportamento atual mantém-se).*

  - **Caso-fronteira — destino é um GRUPO nomeado pelos seus membros.** Pedidos como *"envia 'olá' no grupo da Vera e do André Pinto"* nomeiam um **grupo** (pelos seus participantes), **não** um destinatário 1:1. O comportamento correto é tratá-lo como **envio a grupo**: o assistente **não** passa `intended_recipient`, localiza o grupo cujos membros incluem essas pessoas e segue o `prepare`/`confirm` normal de grupo (resumo com tópico + N participantes + domínios — a barreira anti-erro que mostra quem vai receber antes do `confirm`). A barreira US-3.6 **não dispara** aqui, e é correto não disparar, porque o utilizador endereçou explicitamente o grupo. Se existir **mais do que um grupo** com esses membros, o assistente deve **desambiguar** (mostrar tópico/membros ou perguntar) antes de confirmar; e deve distinguir *"no grupo da Vera e do André"* (um grupo) de *"à Vera e ao André"* (potencialmente dois 1:1 separados) — esclarecendo em caso de ambiguidade.

  - **A garantia é UNIDIRECIONAL (e a falha é sempre segura).** A guarda aplicável pelo servidor só consegue **BLOQUEAR** quando `intended_recipient` é fornecido; o servidor **não distingue por si** "à Vera" (1:1) de "no grupo da Vera e do André" (grupo) — essa é a **classificação de intenção do assistente** (apoiada nas descrições endurecidas das tools, opção A). Logo: a proteção forte é *"se nomeio uma pessoa, nunca cai num grupo"*. O caso inverso (*"se nomeio um grupo, envia ao grupo"*) depende de o assistente **não** marcar `intended_recipient`. Importante: o **modo de falha tende sempre para "não envia"** — se o assistente, por engano, classificar um pedido de grupo como 1:1 e passar `intended_recipient`, o servidor **recusa** (falso bloqueio), nunca havendo fuga. Não existe caminho em que um erro de classificação resulte em envio para o destino errado.
...
| **Fuga de mensagem destinada a uma pessoa para um chat de GRUPO** (partilha de info confidencial com terceiros) | **Alto (RGPD/confidencialidade)** | **US-3.6 / D12:** `intended_recipient` no prepare força verificação server-side de que o `chat_id` é um `oneOnOne` exatamente com essa pessoa; senão **recusa sem token**. Garantia aplicável pelo servidor, não confiança no LLM. Reforçado pelo caminho fácil seguro (US-3.4) e pelas descrições das tools. |
